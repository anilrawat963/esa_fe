...
var n5 = class {
  ...
  setProgress(s3, e4) {
    if (this._progressMap.set(s3, e4), has("enable-feature:esri-3dofl-upload-timings")) {
      ...
      this._startTime ??= r4;
      const o = e(this._timingsMap, s3, () => ({ start: r4, end: 0 }));
      1 === e4 && (o.end = r4);
    }
    this.emitProgress();
  }
...
async function q(e4, { layer: s3, ongoingUploads: t4 }, r4) {
  ...
  if (L(e4, s3)) return r4?.onProgress?.(1), e4;
...
async function H2(e4, s3, r4) {
  const { metadata: o } = e4, { displaySource: n7 } = o, a4 = W(n7?.source, s3, { checkForConversionRequired: !has("disable-feature:georeferenced-uploads") }), i6 = null != a4 ? $(a4, s3, r4) : o.externalSources.length > 0 ? J(e4, s3, r4) : z(e4, s3, r4), c4 = await i6;
...
async function J(e4, s3, t4) {
  ...
  if (!n7) throw new n4();
  const a4 = i4(e3.uploadConvertibleSource, t4?.onProgress, "uploadConvertibleSource"), i6 = { type: "service", assets: await Q(n7, s3, { onProgress: a4.makeOnProgress("uploadEditSource") }) };
  ...
  try {
    ...
    return e4.transform = n8, a5 && (e4.metadata.georeferenced = true, t4?.useAssetOrigin && (e4.vertexSpace.origin = [a5.x, a5.y, a5.z ?? 0], e4.spatialReference = a5.spatialReference)), { source: o2, unitConversionDisabled: true };
  } finally {
    u3.remove();
  }
}
async function z(e4, s3, t4) {
  const r4 = i4(e3.uploadLocalMesh, t4?.onProgress, "uploadLocalMesh"), o = G(e4, s3, { ...t4, onProgress: r4.makeOnProgress("meshToAssetBlob") });
...
async function G(e4, s3, r4) {
  const o = ae(s3), n7 = await e4.load(r4), a4 = await n7.toBinaryGLTF({ origin: n7.origin, signal: r4?.signal, ignoreLocalTransform: true, unitConversionDisabled: true });
...
async function V2(e4, s3, r4) {
  const o = i4(e3.uploadAssetBlobs, r4?.onProgress, "uploadAssetBlobs"), n7 = await Z(e4, s3, { ...r4, onProgress: o.makeOnProgress("prepareAssetItems") });
  s(r4);
...
function Z(e4, s3, r4) {
  const o = i4(e4.length, r4?.onProgress, "prepareAssetItems");
...
async function se({ assets: e4 }, s3, t4) {
  ...
  let o;
  try {
    const e5 = V(s3.parsedUrl.path, "convert3D"), n7 = s3.capabilities?.operations.supportsAsyncConvert3D;
...
//# sourceMappingURL=uploadAssets-EZQDKJG7.js.map
